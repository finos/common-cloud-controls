...
# Inline EKS API + describe permissions for write identity (managed EKS* policies alone are service-role oriented).
EKS_WRITE_INLINE_POLICY_NAME="cfi-eks-write"
EKS_WRITE_INLINE_POLICY='{
  "Version": "2012-10-17",
  "Statement": [
    {
      "Sid": "EKSClusterAPI",
...
ensure_inline_policy() {
  local user_name="$1" policy_name="$2" policy_doc="$3"
  if aws iam get-user-policy --user-name "$user_name" --policy-name "$policy_name" >/dev/null 2>&1; then
    echo "Updating inline policy: $policy_name -> $user_name"
  else
    echo "Creating inline policy: $policy_name -> $user_name"
  fi
...
ensure_inline_policy "$USER_WRITE" "$EKS_WRITE_INLINE_POLICY_NAME" "$EKS_WRITE_INLINE_POLICY"

K8S_API_HOSTNAME="${AWS_K8S_API_HOSTNAME:-}"
if [[ -z "$K8S_API_HOSTNAME" && -f "$TFSTATE" ]] && command -v jq >/dev/null 2>&1; then
  K8S_API_HOSTNAME="$(jq -r '.outputs.kubernetes.value.api_endpoint // .outputs.kubernetes.value.endpoint // empty' "$TFSTATE" | tr -d '\n')"
...

{
  ...
  [[ -n "$K8S_API_HOSTNAME" ]] && printf 'export AWS_K8S_API_HOSTNAME=%q\n' "$K8S_API_HOSTNAME"
  echo ""
} >"$OUT_FILE"
