...
guess_k8s_api_hostname() {
  if [[ -n "${GCP_K8S_API_HOSTNAME:-}" ]]; then
    echo "$GCP_K8S_API_HOSTNAME"
    return
  fi
  local tfstate="$SCRIPT_DIR/../terraform/gcp/terraform.tfstate"
  if [[ -f "$tfstate" ]] && command -v jq >/dev/null 2>&1; then
    local from_state
    from_state="$(jq -r '.outputs.kubernetes.value.api_endpoint // .outputs.kubernetes.value.endpoint // empty' "$tfstate" | tr -d '\n')"
    from_state="${from_state#https://}"
    if [[ -n "$from_state" ]]; then
      echo "$from_state"
      return
    fi
...
K8S_API_HOSTNAME="$(guess_k8s_api_hostname)"

{
  ...
  [[ -n "$K8S_API_HOSTNAME" ]] && printf 'export GCP_K8S_API_HOSTNAME=%q\n' "$K8S_API_HOSTNAME"
