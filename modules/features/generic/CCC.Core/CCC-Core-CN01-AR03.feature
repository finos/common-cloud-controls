...
@PerPort @Behavioural @http @tls @object-storage @virtual-machines @kubernetes
  Scenario: HTTP redirects to HTTPS
    ...
    Given a client connects to "{host-name}" with protocol "http" on port "80"
...
@PerPort @Behavioural @ftp @tls @object-storage @virtual-machines @kubernetes
  Scenario: FTP traffic is blocked or not exposed
    ...
    Given a client connects to "{host-name}" with protocol "ftp" on port "21"
    And I attach "{connection}" to the test output as "FTP response"
    And I refer to "{result}" as "connection"
    Then "{connection}" is an error


@PerPort @Behavioural @telnet @tls @object-storage @virtual-machines @kubernetes
  Scenario: Telnet traffic is blocked or not exposed
    ...
    Given a client connects to "{host-name}" with protocol "telnet" on port "23"
    And I attach "{connection}" to the test output as "Telnet response"
    And I refer to "{result}" as "connection"
    Then "{connection}" is an error


@PerPort @Behavioural @tls @object-storage @virtual-machines @kubernetes
