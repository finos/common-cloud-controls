@CCC.K8S @CCC.K8S.CN09 @CCC.K8S.CN09.AR01 @PerService @tlp-clear @tlp-green @tlp-amber @tlp-red
Feature: CCC.K8S.CN09.AR01 - Keep cluster components within support
  As a security administrator
  I want Kubernetes security controls enforced
  So that managed clusters remain within the approved security boundary

  Background:
    Given a cloud api for "{config}" in "api"
    And I call "{api}" with "GetServiceAPI" using argument "kubernetes"
    And I refer to "{result}" as "kubernetesService"

  @Behavioural @kubernetes @MAIN
  Scenario: Keep cluster components within support
    When I call "{kubernetesService}" with "GetClusterComponentInventory" using argument "{uid}"
    Then "{result}" is not an error
    And I attach "{result}" to the test output as "Cluster component inventory"
    And "{result.ControlPlaneInSupport}" is true
    And "{result.WorkersInSupport}" is true
    And "{result.NodeImageInSupport}" is true
