...
1. Try to identify the cloud service provider from the example.

- If cloud service provider is not identifiable, ask for clarification. Options allowed are AWS, Azure, GCP.

1. Resolve equivalent or closest offerings for AWS, Azure, and GCP.
2. Prefer official documentation domains:

- AWS: `docs.aws.amazon.com`
- Azure: `learn.microsoft.com/azure`
- GCP: `cloud.google.com`

1. If multiple mappings exist, choose the closest functional match and list alternates briefly.
...
1. If the first abbreviation is ambiguous, propose one alternative.
2. Identify the appropriate category(s) for the service based on functionality and existing CCC categories in `catalogs/categories.yaml`. Determine one or more category ids (e.g., `CCC.Storage`).
3. Determine a folder name and full target path for the new service.
4. Validate the proposed category id(s) against `catalogs/categories.yaml`.
5. Determine the folder name and target path internally, but do not create the folder in this step.
6. Folder naming guidance:
...
1. Use `schemas/metadata-schema.json` as the source of truth for required and allowed metadata fields.
2. Build the metadata content from the confirmed inputs and the current schema:
...
1. Validate the final object against `schemas/metadata-schema.json` before writing the file.
2. Write the file to `<confirmed-target-path>/metadata.yaml`.
3. If a `metadata.yaml` already exists, show a diff-style summary and ask for confirmation before overwrite.
...
1. Add matching core capabilities under `imported-capabilities` using the schema shape:
...
1. Add service-specific capabilities under `capabilities` using the schema and style guide only for capabilities that are not already covered by imported core capabilities:
...
1. Follow `style-guides/catalogs/capability-style-guide.yaml` when writing titles and descriptions.
2. Validate the final object against `schemas/capabilities-schema.json` before writing the file.
3. Write the file to `<confirmed-target-path>/capabilities.yaml`.
4. If a `capabilities.yaml` already exists, replace it with the validated content and record the update status.
