...
export const CatalogTypePage: React.FC<Props> = ({ data }) => {
  const { category, service, type, versionPaths, latestData } = data;
  ...
  return (
    <div className="page-layout">
      ...
      <article style={{ flex: 1, minWidth: 0 }}>
        ...
        {versionPaths.length === 0 && <p>No published versions yet.</p>}
          ...
          <>
            <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", marginBottom: "1.5rem", alignItems: "center" }}>
              ...
              {versionPaths.map((vPath, i) => (
                <Link key={vPath} to={vPath} className="catalog-type-btn" style={i > 0 ? { opacity: 0.75 } : undefined}>
                  {vPath.split("/").pop()}{i === 0 ? " (latest)" : ""}
                </Link>
              ))}
            </div>
            {latestData && <CatalogTable data={latestData} />}
          </>
        )}
      </article>
    </div>
