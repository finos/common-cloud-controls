              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>CFI repository</TableHead>
                ...
                <TableBody>
                  {sortedConfigurations.map((config) => {
                    ...
                    const sourceRepoUrl = s?.repository_url ?? config.repository.url;
                    const sourceRepoDescription = s?.repository_description ?? config.repository.description;
                    ...
                    <TableRow key={config.slug}>
                      ...
                      <TableCell>
                        <Link to={`/cfi/${config.slug.split("/")[2]}`} className="text-blue-600 hover:text-blue-800 hover:underline text-sm block">
                          {config.repository.name}
                        </Link>
                      </TableCell>
                      <TableCell className="max-w-xs">
                        <div className="text-sm line-clamp-2 text-muted-foreground" title={sourceRepoDescription}>
                          {sourceRepoDescription}
                        </div>
                        {externalGitUrl && (
                          <a href={externalGitUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 hover:underline text-sm inline-flex items-center gap-1 mt-1">
                            ...
                            {sourceRepoUrl.replace(/^https?:\/\/github\.com\//, "").replace(/\/$/, "")}
                          </a>
                        )}
                      </TableCell>
